...
The browser and the API run on different origins in local development -- the Vite dev server on `http://localhost:5173`, the backend on `http://127.0.0.1:8000` -- so the backend must explicitly allow the frontend's origin. For this ordinary `GET` request, the request still reaches FastAPI and gets a real response; without the right CORS header on that response, the browser blocks the frontend JavaScript from reading it, so the `fetch` call fails from the app's point of view.

`app/main.py` configures `CORSMiddleware` to allow exactly `http://localhost:5173` (Vite's default dev address) and only `GET` and `POST` requests, matching what the frontend currently needs. `POST /expenses` sends a JSON body (`Content-Type: application/json`), which makes the browser send an `OPTIONS` "preflight" request first; unlike the simple `GET` above, if the preflight is not allowed, the browser never sends the real `POST` at all. Starlette's `CORSMiddleware` always allows its own CORS-safelisted request headers -- `Accept`, `Accept-Language`, `Content-Language`, and `Content-Type` -- regardless of `allow_headers`, so `Content-Type` does not need to be opted in explicitly here. If the frontend dev server ever runs on a different port or host, or a future Issue needs another HTTP method or a non-safelisted header from the browser, update the allowed origin/methods/headers there rather than widening it speculatively ahead of need.
