...
The Expense contract constrains the eventual Expense schema -- including an exact decimal `amount`, a date-typed `expense_date`, and a backend-generated `id` -- but it does not choose table names, concrete SQL/ORM column definitions, or other schema details.
